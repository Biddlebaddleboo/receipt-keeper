...
function CardCapture({ index, card, activationReceipts, onPackageFile, onClearPackage, onSideFile, onClearSide, onChange, onExtractPackage, onRetrySide, onRemove }: { index: number; card: CardDraft; activationReceipts: ImageDraft[]; onPackageFile: (file: File, colorMode?: CameraColorMode) => void; onClearPackage: () => void; onSideFile: (side: CardSide, file: File, colorMode?: CameraColorMode) => void; onClearSide: (side: CardSide) => void; onChange: (updates: Partial<CardDraft>) => void; onExtractPackage: () => void; onRetrySide: (side: CardSide) => void; onRemove: () => void }) {
  ...
  const activationOptions = activationReceipts.filter(hasDraftImage).map((receipt) => ({
    receipt,
