...
const port = process.env.RECEIPT_OCR_PORT || "8080";
const dataset = process.env.RECEIPT_OCR_DATASET || "sroie";
const subset = process.env.RECEIPT_OCR_SUBSET || "all";
const limit = process.env.RECEIPT_OCR_LIMIT || "0";
const strategies = process.env.RECEIPT_OCR_STRATEGIES || "";
const outputPath = process.env.RECEIPT_OCR_OUTPUT || `benchmarks/receipt-frontend-ocr-${dataset}-${subset}.json`;
const timeoutMs = Number(process.env.RECEIPT_OCR_TIMEOUT_MS || "3600000");
const debugPort = Number(process.env.RECEIPT_OCR_DEBUG_PORT || "9223");
const chrome = process.env.RECEIPT_CHROME || "/home/ubuntu/.cache/ms-playwright/chromium-1200/chrome-linux/chrome";
const query = new URLSearchParams({ dataset, subset, limit });
if (strategies) query.set("strategies", strategies);
const url = `http://127.0.0.1:${port}/scripts/receipt-frontend-ocr-browser.html?${query}`;
...
const waitForPage = async () => {
  ...
  throw new Error("Chrome did not expose the OCR benchmark page");
};
...
try {
  ...
  chromeProcess = spawn(chrome, [
    ...
    `--user-data-dir=/tmp/receipt-ocr-chrome-${process.pid}`,
    url,
  ], { stdio: "ignore" });
  ...
  if (outputText === "running" || !outputText) throw new Error("OCR benchmark timed out");
  const results = JSON.parse(outputText);
  if (results.error) throw new Error(results.error);
  await writeFile(outputPath, JSON.stringify(results));
  console.log(`Wrote ${results.sampleSize} ${dataset} OCR rows to ${outputPath}`);
} finally {
  socket?.close();
